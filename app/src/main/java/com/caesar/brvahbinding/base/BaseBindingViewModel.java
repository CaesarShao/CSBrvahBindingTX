...
public abstract class BaseBindingViewModel<B> extends BaseViewModel {
    ...
    //设置每个item的占的格数
    public GridSpanSizeLookup spanSizeLookup;//这边这个接口改了，改成3.0的接口了
    //当多布局时,如果item不想继承MultiItemEntity,可以用这个来设置每个itemType
    public BaseMultiTypeDelegate<B> multiTypeDelegat;//这边这个接口改了，改成3.0的了
    //空布局
    ...
    //加载的动画
    public ObservableField<BaseQuickAdapter.AnimationType> animationType;
    //加载的自定义动画
    ...
    public BaseBindingViewModel() {
        ...
        animationType = new ObservableField(BaseQuickAdapter.AnimationType.SlideInBottom);
        ...
        bindingAdapter.setAnimationFirstOnly(isAnimationFirstOnley());//3.0之后，方法名改了
    }
...
//            bindingAdapter.addData(index,newData);
    ...
    public void setSpan(GridSpanSizeLookup spanSizeLookup) {
        this.spanSizeLookup = spanSizeLookup;
    }

    public void setMultiTypeDelegat(BaseMultiTypeDelegate<B> multiTypeDelegat) {
