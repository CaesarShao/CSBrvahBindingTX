...
//            bindingAdapter.getDraggableController().disableDragItem();
            bindingAdapter.getDraggableModule().setDragEnabled(false);//3.0版本之后,改变
        } else {
            isDrag = true;
//            bindingAdapter.getDraggableController().enableDragItem(bindingAdapter.getItemTouchHelper(null));
            bindingAdapter.getDraggableModule().setDragEnabled(true);
        }
    }
...
//            bindingAdapter.getDraggableController().disableSwipeItem();
            bindingAdapter.getDraggableModule().setSwipeEnabled(false);
        } else {
            isSwipe = true;
//            bindingAdapter.getDraggableController().enableSwipeItem();
            bindingAdapter.getDraggableModule().setSwipeEnabled(true);
        }
    }
}
